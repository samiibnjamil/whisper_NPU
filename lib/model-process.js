...
// Module-level map: "engineKey:modelKey" -> { state, child, subscribers }
// Persists across API requests within the same Node process.
const sessions = {};

function sessionKey(engineKey, modelKey) {
  return `${engineKey}:${modelKey}`;
}

export function getModelStatus(engineKey, modelKey) {
  const s = sessions[sessionKey(engineKey, modelKey)];
  if (!s) return "idle";
...
export function loadModel({ engineKey, modelKey, scriptName, modelFolder, projectRoot, onEvent }) {
  const key = sessionKey(engineKey, modelKey);
  ...
  if (existing?.state === "loading") {
    ...
    onEvent({ type: "stage", stage: "loading_model", message: "Loading model (already in progress)" });
    return () => {
...
export function unloadModel(engineKey, modelKey) {
  const key = sessionKey(engineKey, modelKey);
